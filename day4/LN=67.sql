...
-- link https://www.sql-ex.ru/learn_exercises.php?LN=67

-- Solution
SELECT TOP 1 MAX(COUNT(*)) OVER() AS max_res  FROM
(
  SELECT 
    CASE 
    ...
    END as point_2
  FROM 
    ( SELECT DISTINCT id_comp, town_from , town_to from trip ) as trip
) AS tab
