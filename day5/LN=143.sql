...
-- link https://www.sql-ex.ru/learn_exercises.php?LN=143
-- Solution
SELECT name, CONVERT(DATE, date, 101), 
case 
when DATEPART(weekday,eomonth(date)) = 7 then DATEADD(day, -1, eomonth(date))
when DATEPART(weekday,eomonth(date)) = 6 then DATEADD(day,  0, eomonth(date)) 
when DATEPART(weekday,eomonth(date)) = 5 then DATEADD(day, -6, eomonth(date)) 
when DATEPART(weekday,eomonth(date)) = 4 then DATEADD(day, -5, eomonth(date))  
when DATEPART(weekday,eomonth(date)) = 3 then DATEADD(day, -4, eomonth(date))  
when DATEPART(weekday,eomonth(date)) = 2 then DATEADD(day, -3, eomonth(date))  
when DATEPART(weekday,eomonth(date)) = 1 then DATEADD(day, -2, eomonth(date)) end  
from battles
