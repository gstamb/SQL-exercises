...
-- Adding a filter for empty or Null string or Trimming does not seem to resolve it. No idea how to resolve.
-- The advise is to consider empty string as input but my where filter should be able to catch that ? right ?


-- link https://www.sql-ex.ru/learn_exercises.php?LN=148
-- Solution
SELECT DISTINCT ship as ship,
CONCAT(
left(ship, CHARINDEX( ' ', ship)),
replicate('*', len(ship) - CHARINDEX(' ', ship) - CHARINDEX(' ', reverse(ship))),
    right(ship, CHARINDEX( ' ', reverse(ship)))) as new_name
from outcomes
where DATALENGTH(ship)-LEN(REPLACE(ship,' ', '')) >=  2
