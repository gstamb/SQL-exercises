...
-- link https://www.sql-ex.ru/learn_exercises.php?LN=98

-- Solution
with cte as (
SELECT 
CAST((speed|ram) as integer) as n,
code as c, 
cast(''  as varchar(MAX)) as binary from pc 
UNION all
SELECT
n / 2 ,
c,
CAST((n % 2) AS VARCHAR(MAX)) + binary from cte where n > 0)
select cte.c, pc.speed, pc.ram from cte, pc
where cte.c = pc.code and cte.n = 0 and cte.binary like '%1111%'
