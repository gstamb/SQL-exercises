...
-- link https://www.sql-ex.ru/learn_exercises.php?LN=112

-- Solution
SELECT case when count(*) = 3 then min(d.rest) else 0 end from(
SELECT sum(rest)/255 as rest FROM (
SELECT v.v_id id, v.v_color color, Coalesce(255-sum(b.b_vol),255) rest from utv v
